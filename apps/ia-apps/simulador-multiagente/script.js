(() => {
  ...
  const STORAGE_KEY = "simulador_multiagente_gemini_api_key";
  const LEGACY_STORAGE_KEY = "narrativa_emergente_gemini_api_key";
  ...
  function init() {
    ...
    const savedKey = localStorage.getItem(STORAGE_KEY) || localStorage.getItem(LEGACY_STORAGE_KEY);
    if (savedKey) {
  ...
  async function runTurn(origin) {
    ...
    try {
      ...
      if (shouldPauseSimulation(error, state.consecutiveErrors)) {
        stopSimulation();
        addLog("Simulacion en pausa para evitar reintentos continuos. Revisa la API Key y presiona Guardar para reanudar.", "error");
        showApiModal();
      }
    } finally {
  ...
  async function askGeminiForAction(character) {
    ...
    if (!response.ok) {
      ...
      const error = new Error(`HTTP ${response.status}: ${errorText.slice(0, 180)}`);
  ...
  function shouldPauseSimulation(error, consecutiveErrors) {
