...
function attachEventListeners() {
    ...
    if (elements.btnUploadTrigger && elements.fileUpload) {
        console.log('✅ Elementos de carga de archivo encontrados');
        elements.btnUploadTrigger.addEventListener('click', () => {
            console.log('🖱️ Click en botón de subir archivo');
        ...
        console.error('❌ No se encontraron elementos de carga:', {
            btnUploadTrigger: !!elements.btnUploadTrigger,
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log('🚀 Iniciando aplicación Constructor de Ejercicios');
    initializeElements();
    console.log('✅ Elementos inicializados');
    attachEventListeners();
    console.log('✅ Event listeners adjuntados');
    await loadMasterPrompt(); // Cargar prompt desde archivo
    console.log('✅ Prompt maestro cargado');
    loadFromLocalStorage();
    renderApp();
    console.log('✅ Aplicación renderizada y lista');
});
...
function handleFileUpload(event) {
    ...
    console.log('📂 Archivo detectado:', file.name, file.type, file.size, 'bytes');
...
function handleJsonAction(action) {
    console.log('🎯 handleJsonAction llamada con acción:', action);
    const rawText = elements.jsonInput.value.trim();
    if (!rawText) {
        showStatus('Entrada vacía. Pega un JSON o sube un archivo.', 'error');
        return;
    }
    
    console.log('📝 Texto a procesar (primeros 100 caracteres):', rawText.substring(0, 100));
    ...
    try {
        ...
        if (action === 'load') {
            ...
            console.log('📥 Datos cargados en currentData');
        } else {
            mergeExercises(validatedData.exercises);
            console.log('🔀 Ejercicios fusionados');
        }
        renderApp();
        saveToHistory();
        autoSave();
        ...
        console.error('❌ Error en handleJsonAction:', e);
