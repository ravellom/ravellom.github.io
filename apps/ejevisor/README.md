# 🎮 EjeVisor - Visor Interactivo de Ejercicios

Aplicación gamificada para visualizar y resolver ejercicios generados con **[EjeCon](../ejecon/)**.

## 🌟 Características Principales

### 🎯 Experiencia Gamificada
- **Sistema de puntos XP**: Gana puntos por cada respuesta correcta
- **Rachas (streaks)**: Bonificación por respuestas consecutivas correctas
- **Barra de progreso**: Visualiza tu avance en tiempo real
- **Efectos visuales**: Confetti y animaciones en aciertos
- **Feedback inmediato**: Retroalimentación educativa completa

### 🎨 3 Temas Visuales
1. **🎨 Modo Gamer** (por defecto): Colores vibrantes y estilo lúdico
2. **🧘 Modo Zen**: Diseño minimalista y relajante
3. **🌙 Modo Oscuro**: Pantalla oscura que cuida tus ojos

### 📚 Tipos de Ejercicios Soportados
- ✅ **Multiple Choice** - Elección múltiple
- ✅ **True/False** - Verdadero o Falso
- ✅ **Fill Gaps** - Rellenar huecos
- ✅ **Ordering** - Ordenar secuencias (arrastrables)
- ✅ **Matching** - Emparejar elementos (arrastrables)
- ✅ **Grouping** - Clasificar en categorías (arrastrables)
- ✅ **Short Answer** - Respuesta corta
- ✅ **Hotspot** - Zonas clicables en imagen
- ✅ **Slider** - Escala numérica

---

## 🚀 Cómo Usar

### 1️⃣ Cargar Ejercicios

**Opción A: Desde archivo**
1. Clic en la zona de carga o arrastre un archivo `.json`
2. Seleccionar archivo generado con EjeCon
3. ¡Comenzar a resolver!

**Opción B: Ejemplo de prueba**
1. Clic en "Cargar Ejemplo de Prueba"
2. Se cargan ejercicios de demostración
3. Úsalo para familiarizarte con la interfaz

### 2️⃣ Resolver Ejercicios

1. **Lee el enunciado** en la parte superior
2. **Responde** según el tipo de ejercicio:
   - Elección: Clic en la opción correcta
   - Huecos: Escribe en los espacios
   - Ordenar: Arrastra elementos a su posición
   - Emparejar: Arrastra elementos al lugar correcto
   - Clasificar: Arrastra a la categoría correspondiente
3. **Comprueba** con el botón azul
4. **Lee el feedback** con explicación pedagógica
5. **Siguiente** para continuar

### 3️⃣ Navegación

- **⬅️ Anterior**: Revisar ejercicio previo
- **➡️ Siguiente**: Avanzar sin comprobar
- **💡 Ver Pista**: Obtener ayuda (si disponible)
- **🔄 Nuevo archivo**: Cargar otros ejercicios

---

## 🎯 Sistema de Puntuación

### Puntos Base
- **Correcta primera vez**: +100 XP
- **Correcta segundo intento**: +50 XP
- **Tercera vez o más**: +25 XP

### Bonificación por Racha
- **2 consecutivas**: +20 XP
- **3 consecutivas**: +30 XP  
- **5 consecutivas**: +50 XP
- **8+ consecutivas**: +100 XP 🔥

### Penalizaciones
- **Respuesta incorrecta**: Sin puntos
- **Racha rota**: Vuelve a 0

---

## 🎨 Personalización

### Cambiar Tema
Selector en la esquina superior derecha:
- 🎨 Modo Gamer - Colorido y vibrante
- 🧘 Modo Zen - Minimalista y sereno
- 🌙 Modo Oscuro - Suave con la vista

### Configuración Visual
Cada tema ajusta automáticamente:
- Colores de fondo y texto
- Bordes y sombras
- Radios de bordes
- Efectos de hover

---

## 📁 Formato JSON Compatible

EjeVisor usa **el mismo formato que EjeCon**:

```json
{
  "resource_metadata": {
    "title": "Título del recurso",
    "topic": "Tema o asignatura"
  },
  "exercises": [
    {
      "id": "ex_TIMESTAMP_INDEX",
      "type": "tipo_ejercicio",
      "content": {
        "prompt_text": "Enunciado del ejercicio"
      },
      "interaction": {
        // Datos específicos del tipo
      },
      "scaffolding": {
        "hint_1": "Pista",
        "explanation": "Explicación",
        "learn_more": "Contenido ampliado"
      }
    }
  ]
}
```

---

## 🔗 Integración con EjeCon

### Flujo de Trabajo Completo

```
EjeCon (Creación)          EjeVisor (Práctica)
      ↓                           ↓
1. Genera con IA          4. Carga JSON
2. Edita ejercicios       5. Resuelve ejercicios
3. Exporta JSON    →      6. Aprende con feedback
```

### Compatibilidad Total
- ✅ Misma estructura JSON
- ✅ Mismo schema de validación
- ✅ Mismos 9 tipos de ejercicios activos en EjeCon
- ✅ Mismo scaffolding DUA

---

## 🛠️ Características Técnicas

### Tecnologías
- **Vanilla JavaScript** (sin frameworks)
- **SortableJS** para drag & drop
- **Canvas Confetti** para efectos visuales
- **Phosphor Icons** para iconografía
- **CSS Variables** para temas dinámicos

### Arquitectura
- **SPA** (Single Page Application)
- **3 pantallas**: Upload, Game, Results
- **Estado global** con objeto `state`
- **Renderizado dinámico** según tipo

### Renderizadores Específicos
Cada tipo de ejercicio tiene su propio renderizador:
- `renderMultipleChoice()` / `renderTrueFalse()`
- `renderFillGaps()`
- `renderOrdering()` con SortableJS
- `renderMatching()` con drag & drop
- `renderGrouping()` con drop zones
- `renderShortAnswer()`
- `renderHotspot()`
- `renderSlider()`

### Validadores
- `checkMultipleChoice()` - Compara ID seleccionado
- `checkFillGaps()` - Normaliza y compara texto
- `checkOrdering()` - Verifica orden correcto
- `checkMatching()` - Valida parejas
- `checkGrouping()` - Comprueba categorías

---

## 📚 Scaffolding DUA Implementado

### hint_1 (Pista)
- Botón "💡 Ver Pista" (se habilita tras 1er intento fallido)
- Muestra ayuda sin revelar la respuesta
- Diseñada pedagógicamente

### explanation (Explicación)
- Se muestra en el modal de feedback
- Explica el **por qué** de la respuesta correcta
- Refuerza el aprendizaje conceptual

### learn_more (Ampliar)
- Contenido adicional opcional
- Ejemplos, curiosidades, contexto
- Profundización para estudiantes interesados

---

## 🎓 Uso Educativo

### Para Estudiantes
- **Autoevaluación**: Practica a tu ritmo
- **Feedback inmediato**: Aprende de errores
- **Gamificación**: Motivación por puntos/rachas
- **Variedad**: Diferentes tipos de interacción

### Para Docentes
- **Fácil distribución**: Comparte archivo JSON
- **Sin instalación**: Solo navegador web
- **Tracking visual**: Estudiante ve su progreso
- **Versátil**: Funciona en PC, tablet, móvil

### Para Contenidistas
- **Previsualización**: Prueba ejercicios antes de publicar
- **QA rápido**: Identifica problemas de diseño
- **Sin backend**: Funciona offline con archivos locales

---

## 🐛 Solución de Problemas

### No carga el archivo JSON
- ✅ Verifica que sea JSON válido
- ✅ Comprueba que tenga campo `exercises`
- ✅ Revisa la consola del navegador (F12)

### Drag & drop no funciona
- ✅ Solo en ejercicios ordering, matching, grouping
- ✅ Usa navegador moderno (Chrome, Firefox, Edge)
- ✅ Verifica que SortableJS esté cargado

### Los ejercicios no se muestran bien
- ✅ Verifica estructura del JSON
- ✅ Comprueba campo `interaction` según tipo
- ✅ Usa ejemplo.json como referencia

---

## 📄 Archivos del Proyecto

```
ejevisor/
├── index.html          # Interfaz principal (con topbar RecuEdu)
├── visor.html          # Versión standalone (sin topbar)
├── visor.js            # Lógica principal
├── visor.css           # Estilos y temas
├── ejemplo.json        # Ejercicios de demostración
└── README.md           # Esta documentación
```

---

## 🔗 Enlaces Relacionados

- **[EjeCon](../ejecon/)** - Crea ejercicios con IA
- **[RecuEdu Labs](../../)** - Más herramientas educativas
- **[GitHub](https://github.com/ravellom)** - Repositorio del proyecto

---

## 📝 Notas de Versión

### v1.0.0 (Febrero 2026)
- ✅ Compatibilidad total con EjeCon
- ✅ Corrección problema topbar
- ✅ Botón "Cargar Ejemplo"
- ✅ 9 tipos de ejercicios soportados
- ✅ 3 temas visuales
- ✅ Sistema de gamificación completo
- ✅ Documentación completa

---

**Desarrollado por**: RecuEdu Labs  
**Licencia**: Open Source Educativo  
**Última actualización**: Febrero 5, 2026
